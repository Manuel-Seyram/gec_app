...
mixin _Bar on _State {
  Widget sliverPersistentHeader() {
    return new SliverPersistentHeader(
        pinned: true,
        floating: false,
        delegate: new ScrollPageBarDelegate(
            Navigator.canPop(context) ? _barPopup : _barPage,
            maxHeight: widget.barMaxHeight)
        // delegate: new ScrollPageBarDelegate(_barPopup,maxHeight: widget.barMaxHeight)
        );
  }

  Widget _barPopup(BuildContext context, double offset, bool overlaps,
  ...
  Widget _barPage(BuildContext context, double offset, bool overlaps,
      double shrink, double stretch) {
    ...
    return Stack(children: <Widget>[
      ...
      Align(
          alignment: Alignment.lerp(
              Alignment(-0.5, 0.5), Alignment(-0.7, -.1), stretch),
  ...
  Widget _barTitle(double shrink) {
    return Text(widget.title ?? core.appName,
