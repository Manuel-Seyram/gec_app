...
mixin _Bar on _State {
  Widget sliverPersistentHeader() {
    return new SliverPersistentHeader(
        ...
        floating: true,
        delegate:
            new ScrollPageBarDelegate(_bar, minHeight: 40, maxHeight: 50));
  }

  Widget _bar(BuildContext context, double offset, bool overlaps, double shrink,
      double stretch) {
  ...
  Widget input(double shrink, double stretch) {
    ...
    return TextFormField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: Icon(CustomIcon.find,
              color: Colors.grey[focusNode.hasFocus ? 100 : 400], size: 20),
          // prefixIcon: Icon(Icons.search,color:Colors.grey[focusNode.hasFocus?100:400],size: 22),
          hintText: " ...search Verse",
          // hintStyle: TextStyle(color: Colors.grey),
          contentPadding:
              EdgeInsets.symmetric(horizontal: 1, vertical: (3 * shrink)),
          // fillColor: Color(0xffeff1f4).withOpacity(shrink),
          fillColor:
              Colors.grey[focusNode.hasFocus ? 300 : 200].withOpacity(shrink),
          ...
          focusedBorder: OutlineInputBorder(
            borderSide: BorderSide(
                color: Colors.grey[300].withOpacity(shrink), width: 0.1),
            borderRadius: BorderRadius.all(Radius.circular(100)),
          ),
          enabledBorder: OutlineInputBorder(
            borderSide: BorderSide(
                color: Colors.grey[200].withOpacity(shrink), width: 0.1),
