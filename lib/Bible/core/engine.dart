...
Collection parseCollectionCompute(dynamic response) {
  Map<String, dynamic> parsed =
      (response is String) ? decodeJSON(response) : response;
  return Collection.fromJSON(parsed)
    ..bible.sort((a, b) => a.order.compareTo(b.order));
}
...
Future<String> loadBundleAsString(String fileName) =>
...
Future<File> documents(String fileName) async =>
    new File(join(await directory.then((e) => e.path), fileName));

Future<File> docsWrite(String fileName, String fileContext) async =>
    await documents(fileName)
        .then((File e) async => await e.writeAsString(fileContext));

Future<String> docsRead(String fileName) async =>
    await documents(fileName).then((File e) async => await e.readAsString());

Future<FileSystemEntity> docsDelete(String fileName) async =>
    await documents(fileName).then((File e) async => await e.delete());

/// NOTE: if exist return path basename,if not NULL
Future<String> docsExists(String fileName) async => await documents(fileName)
    .then((File e) async => await e.exists() ? basename(fileName) : null);
