...
mixin _Bookmark on _Bible {
  ...
  Future<bool> addBookmarkTest() async =>
      await readCollection().then((o) async {
        int total = o.bookmark.length;
        int index = o.bookmark.indexWhere(
            (i) => i.bookId == this.bookId && i.chapterId == this.chapterId);
        bool hasNotBookmarked = index < 0;
        if (hasNotBookmarked) {
          if (isBookmarkMounted)
            this.keyBookmarkList.currentState.insertItem(total);
          o.bookmark.add(CollectionBookmark(
              bookId: this.bookId, chapterId: this.chapterId));
        } else {
          ...
          if (isBookmarkMounted)
            this
                .keyBookmarkList
                .currentState
                .removeItem(index, (_, __) => null);
        }
        await writeCollection();
        return hasNotBookmarked;
      });

  Future<bool> removeBookmark(int index) async =>
      await readCollection().then((o) async {
  ...
  Future<bool> hasBookmark() async => await readCollection().then((o) =>
      o.bookmark.firstWhere(
