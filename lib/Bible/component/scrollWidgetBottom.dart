...
class _BottomBarAnimatedState extends State<ScrollPageBottom>
  ...
  int milliseconds(double heightFactor) =>
      [0.0, 1.0].contains(heightFactor) ? 200 : 0;
  ...
  Widget _item(BuildContext context, int value, Widget child) {
    return Row(
        ...
        children: items
            .asMap()
            .map((index, item) =>
                MapEntry(index, _button(index, item, value == index)))
            .values
            .toList());
  }

  Widget _animatedContainer(
      BuildContext context, double heightFactor, Widget children) {
    // return Align(
    //   heightFactor: 1.0,
    //   child: Stack(
